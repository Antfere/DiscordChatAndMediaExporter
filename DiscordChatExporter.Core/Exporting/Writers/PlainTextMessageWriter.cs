...
internal class PlainTextMessageWriter : MessageWriter
{
        ...
        CancellationToken cancellationToken = default)
    {
        foreach (var embed in embeds)
        {
            ...
            // Archiving embed (Will write url to text file if not downloading media. Thus doubling up url, otherwise will write relative path to archived media):
            if (!string.IsNullOrWhiteSpace(embed.Url))
            {
                if (!string.IsNullOrWhiteSpace(embed.Thumbnail?.Url ?? embed.Thumbnail?.ProxyUrl))
                    await _writer.WriteLineAsync(await Context.ResolveMediaUrlAsync(embed.Url, cancellationToken, (embed.Thumbnail?.ProxyUrl ?? embed.Thumbnail?.Url) ?? ""));
                else
                {
                    await _writer.WriteLineAsync(await Context.ResolveMediaUrlAsync(embed.Url, cancellationToken));
                }
            }
